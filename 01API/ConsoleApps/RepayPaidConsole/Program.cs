...
using static RepayPaidConsole.RepayDTO;
...
public class Program
{
            ...
            if (commaSepartedDBs != null && commaSepartedDBs.Count() > 0)
            {
                foreach (var db in commaSepartedDBs)
                {
                    ...
                    try
                    {
                        ...
                        string? RepayURL = configuration.GetSection("RepayGetProcessedListAPI").Value;
                        //string? payableURL = configuration.GetSection("PayableUrl").Value;
                        string? payableURL = "https://localhost:5264";
                       // await CallGetOpenRepayments(payableURL, RepayURL, "","",token,"");
    ...
    static async Task CallGetOpenRepayments(string? baseUrl, string? endPointURl, string clientId, string clientScrectId, string token, string urlName)
    {
            ...
            if (restOptions != null)
            {
                using (var client = new RestClient(restOptions))
                {
                    try
                    {
                        ...
                        GetRepayProcessedList req = new GetRepayProcessedList();
                        req.StDate = DateTime.Now.AddDays(-2);
                        req.EdDate = DateTime.Now.AddDays(1);
                        ...
                        if (resp != null && resp.IsSuccessful)
                        {
                            if (token == null)
                            {
                                var tokenRes = JsonConvert.DeserializeObject<List<GetRepayProcessedListResponse>>(resp.Content);
