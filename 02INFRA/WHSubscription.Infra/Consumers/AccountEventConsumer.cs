...
{
    public class AccountEventConsumer : IConsumer<AccountMessageEvent>
    {
        #region Fields
        private readonly IUnitOfWork unitOfWork;
        private readonly ILogger<AccountEventConsumer> logger;
        ...
        #region Constructor
        public AccountEventConsumer(IUnitOfWork unitOfWork,
        ...
        #region Methods
        public async Task Consume(ConsumeContext<AccountMessageEvent> context)
        {
            AccountMessageEvent message = context.Message;
            try
            {
                ...
                foreach (var subscription in subscriptions)
                    {
                        bool failed = false;
                        var payload = JsonSerializer.Serialize(message.AccountEventPayload);
                        ...
                        try
                        { 
                            var response = await httpClient.PostAsJsonAsync(urlName, message.AccountEventPayload);
                        ...
                        { 
                            ...
                            AccountReattemptEvent request = MapRequest(message, urlName, subscription.Id);
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex, "Error processing AccountEvent");
            }
        }
        private static AccountReattemptEvent MapRequest(AccountMessageEvent message, string URLName, long subscriptionID) => new()
        {
            AccountEventPayload = message.AccountEventPayload,
            UrlName = URLName,
    ...
    public class AccountReattemptConsumer : IConsumer<AccountReattemptEvent>
    {
        #region Fields
        private readonly ILogger<AccountReattemptConsumer> logger;
        private readonly IConsumerLog consumerLog;
        private readonly string externalAPI_SecretKey;
        #endregion

        #region Constructor
        public AccountReattemptConsumer(ILogger<AccountReattemptConsumer> _logger,
        ...
        #region Methods
        public async Task Consume(ConsumeContext<AccountReattemptEvent> context)
        {
            AccountReattemptEvent message = context.Message;
            ...
            if (message != null && message.UrlName != null)
            {
                ...
                try
                {
                    ...
                    httpClient.DefaultRequestHeaders.Remove("X-Signature");
                    httpClient.DefaultRequestHeaders.Add("X-Signature", signature);
                    var response = await httpClient.PostAsJsonAsync(message.UrlName, message.AccountEventPayload);
                    if (response.StatusCode != System.Net.HttpStatusCode.OK && response.StatusCode != System.Net.HttpStatusCode.NoContent)
                    {
                        string exMsg = (response.StatusCode == System.Net.HttpStatusCode.BadRequest || response.StatusCode == System.Net.HttpStatusCode.Unauthorized) ? "Invalid signature" : "";
            ...
            {
                logger.LogWarning("Received null message in Account ReattemptConsumer");
            }
        }
        #endregion
    }

    public class AccountEventErrorConsumer : IConsumer<Fault<AccountReattemptEvent>>
    {
        private readonly IConsumerLog log;
        public AccountEventErrorConsumer(IConsumerLog log)
        {
            this.log = log;
        }
        public async Task Consume(ConsumeContext<Fault<AccountReattemptEvent>> context)
        {
            AccountReattemptEvent message = context.Message.Message;
